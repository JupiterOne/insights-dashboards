...
exports.afterDeployTerraform = async (env) => {
  const S3_BUCKET = `${env["name"]}-insights-dashboards`;
  const AWS = baseDeploy.getAws().getAwsSdk();
  const s3 = new AWS.S3();
  const config = baseDeploy.getConfig();
  const distDir = config.getProjectDir() + "/dist/";
  const assets = [];
  ...
  await walkDir(distDir, {
    async onFile(file, stat) {
      ...
      if (!file.startsWith(distDir)) {
        throw new Error(
          `The file ${file} did not start with the expected path ${distDir}`
        );
      }
      assets.push({
        fullPath: file,
        relativePath: file.substring(distDir.length),
  ...
  for (const obj of s3Objects) {
    try {
      await fs.access(distDir + obj.Key);
    } catch (err) {
